...
// Aggregate files for term
#[get("/aggregate/{term}-{count}")]
async fn aggregate_search(app_state: Data<Mutex<AppState>>, path: Path<(String, usize)>) -> impl Responder {
    let mut app_state = app_state.lock().unwrap();
    app_state.update();

    let args = path.into_inner();
    match app_state.aggregator.aggregate(args.0.clone(), args.1) {
        Ok(_) => {
            return HttpResponse::Ok().finish();
        }
        Err(e) => {
            warn!("Failed to aggregate for term \"{term}\": {e}", term = args.0);
